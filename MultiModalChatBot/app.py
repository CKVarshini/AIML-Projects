    ...
    if st.session_state.history:  # Check if history is not empty
        if st.session_state.session_key == "new_session":
            ...
            save_chat_history_json(
                st.session_state.history, config["chat_history_path"] + st.session_state.new_session_key)
        else:
            save_chat_history_json(
                st.session_state.history, config["chat_history_path"] + st.session_state.session_key)


def main():
    st.title("Intelligent Powered Education System")
    chat_cotainer = st.container()
    ...
    st.sidebar.selectbox("Select a chat session", chat_sessions,
                         key="session_key", index=index, on_change=track_index)
